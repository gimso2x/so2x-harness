...
`project_profiles.py`는 단순 profile bundle이 아니라 외부 skill metadata catalog(`templates/project/.ai-harness/skill-catalog.json`)를 사용합니다. catalog 구조는 `schemas/skill-catalog.schema.json`으로 검증합니다.
